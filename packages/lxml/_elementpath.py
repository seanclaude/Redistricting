    ...
    if signature == "-" or signature == "-()" or signature == "-()-":
        ...
        if signature == "-":
            ...
            if index < 0:
                if index == -1:
                    raise SyntaxError(
                        "indices in __path predicates are 1-based, not 0-based")
                else:
                    raise SyntaxError("__path index >= 1 expected")
        else:
            if predicate[0] != "last":
...
def _build_path_iterator(path, namespaces):
    ...
    if path[:1] == "/":
        raise SyntaxError("cannot use absolute __path on element")
    ...
    try:
        ...
        raise SyntaxError("empty __path expression")
    selector = []
    while 1:
        try:
            selector.append(ops[token[0]](_next, token))
        except StopIteration:
            raise SyntaxError("invalid __path")
        try:
            token = _next()
            if token[0] == "/":
                token = _next()
